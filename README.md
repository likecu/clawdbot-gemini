...
│   ├── Dockerfile.node        # Node.js 版本镜像定义
│   ├── Dockerfile.official    # 官方 OpenCode 镜像定义
│   ├── opencode_server.py     # Flask 服务端实现
│   ├── opencode_requirements.txt  # OpenCode Python 依赖
...
#### OpenCode 客户端（opencode.py）

OpenCode 客户端封装了与 OpenCode 服务交互的所有功能，提供与 OpenAI API 兼容的接口。该模块的设计目标是让业务层可以像调用普通 API 一样调用 OpenCode 服务，无需关心底层的 HTTP 通信细节和认证机制。

`OpenCodeClient` 类实现了请求构造、认证头添加、响应解析和错误重试等功能。客户端维护了请求超时配置和重试策略，默认超时时间为 30 秒，最大重试次数为 3 次。请求失败时会自动进行指数退避重试，有效应对临时性的网络故障和服务不可用情况。

客户端支持所有 OpenAI 兼容的 API 端点，包括聊天完成（/v1/chat/completions）、模型列表（/v1/models）等。响应格式完全遵循 OpenAI API 规范，可以与支持 OpenAI API 的各种工具和库无缝对接。这种设计大大提高了系统的兼容性，也为将来可能的模型切换提供了便利。
...
#### 服务端主程序（opencode_server.py）

OpenCode 服务端主程序是基于 Flask 框架实现的 Web 服务器，提供 OpenAI 兼容的 API 接口。服务器监听 8080 端口，接收来自 Clawdbot 服务的 HTTP 请求，执行代码生成和执行操作，然后返回结构化的响应结果。服务端的设计充分考虑了安全性，实现了请求认证、输入验证和输出过滤等多重安全机制。

服务器使用 Flask 框架的蓝图（Blueprint）功能组织路由，将不同功能的路由分组管理。主要端点包括健康检查端点（/health）、模型列表端点（/v1/models）和聊天完成端点（/v1/chat/completions）。每个端点都有独立的处理函数，函数内部实现了完整的请求验证、业务逻辑处理和响应格式化流程。

认证中间件是服务器安全的核心组件，通过 `authenticate_request` 装饰器实现。所有业务端点都需要经过认证检查，请求头中必须包含有效的 Authorization 字段，Token 必须与配置的 API Key 匹配。认证失败会返回 401 状态码，并附带详细的错误描述，便于客户端进行问题排查。

#### 代码执行器（executor.py）

代码执行器是 OpenCode 服务的安全核心组件，负责分析和执行用户提交的代码。该模块实现了多语言支持、代码安全检测、临时文件管理和执行结果捕获等功能。`CodeExecutor` 类封装了所有代码执行相关的逻辑，通过语言检测、执行和结果收集三个阶段完成代码执行任务。

语言检测功能通过分析代码的语法特征来识别编程语言类型。当前支持 Python、JavaScript 和 Bash 三种语言，未来可以通过配置文件扩展支持的语言列表。每种语言都有对应的执行环境和安全策略，Python 使用子进程执行，JavaScript 使用 Node.js 运行时，Bash 使用系统 shell 执行。

安全执行策略包括代码内容扫描、危险命令过滤和执行时间限制。代码提交后会首先经过正则表达式扫描，识别可能存在安全风险的代码模式，如文件操作命令、系统调用等。危险命令会被拦截并返回错误提示。执行过程有超时保护，超时后会强制终止进程并返回超时错误。这些机制确保了代码执行不会对宿主系统造成损害。

执行结果收集包括标准输出、标准错误和退出码三个维度。执行完成后会捕获所有输出内容，包括程序打印的信息和错误信息。退出码用于判断程序是否正常结束，非零退出码表示程序运行出错。结果会经过格式化处理，以结构化的方式返回给调用方，同时清理执行过程中产生的临时文件。

#### Gemini 集成模块（llm.py）

OpenCode 服务的 Gemini 集成模块与 Clawdbot 服务中的同名模块功能类似，但针对代码生成场景进行了优化。模块提供了 `init_gemini` 和 `get_response_with_history` 两个核心函数，后者支持对话历史管理，可以根据上下文生成更加连贯的代码。

模型初始化从环境变量中读取 Google API Key，创建 genai.Client 实例。客户端实例会被缓存到 `CodeExecutor` 对象中，供后续的代码生成调用使用。初始化失败时会打印警告信息并设置 `gemini_model` 为 None，此时系统会降级为简单的代码执行模式，放弃智能代码生成功能。

代码生成调用使用 `client.models.generate_content` 方法，传入代码请求和上下文信息。请求提示词经过精心设计，引导模型生成符合要求的代码片段。响应处理会提取代码内容，必要时进行语法高亮和格式化处理。模型调用过程中的异常会被捕获并记录日志，但不会影响代码执行流程的继续执行。
...
检查 OpenCode 服务是否正常运行，确认 Google API Key 配置正确且有足够配额。代码执行超时时间默认为 30 秒，如果代码需要更长时间运行，可以修改 `opencode/src/executor.py` 中的超时配置。
