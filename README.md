...
为了支持多平台（如 QQ、飞书）的统一消息处理，系统引入了 Channel 架构：
1. **UnifiedMessage**: 抽象了不同平台的消息格式，统一包含 user_id, chat_id, content, images 等字段。
2. **ChannelManager**: 负责管理所有通道的生命周期（启动、停止）及消息路由。
3. **平台适配器**: 
   - `LarkChannel`: 封装飞书 SDK。
   - `QQChannel`: 封装 NapCat(OneBot11) 接口。

### 优化与修复记录
- **架构精简**: 移除了 Redis 依赖，全面转向内存存储 (Memory Storage) 管理会话，降低了部署复杂度和资源占用。
- **OCR 视觉能力**: 集成了 Google Gemini Vision API，支持 QQ 和飞书渠道的图片识别。用户发送图片时，系统会自动提取文字和视觉信息作为上下文。
- **调试增强**: 新增 `/debug` 指令，可实时查看系统生成的 Prompt 和上下文信息，便于开发调试。
- **重复消息过滤**: 在 QQ 通道中，通过 WebSocket 事件过滤机制，仅处理 `message` 事件而忽略 `message_sent` 事件。
- **即时响应**: 引入了即时确认机制，提升用户交互体验。
