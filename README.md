...
### 优化与修复记录
- **重复消息过滤**: 在 QQ 通道中，通过 WebSocket 事件过滤机制，仅处理 `message` 事件而忽略 `message_sent` 事件，彻底解决了机器人自回复导致的重复处理问题。
- **即时响应确认**: 引入了“收到你的问题啦，我正在思考哦”的即时确认机制，提升用户交互体验。
- **消息过滤机制**: 实现了自动清除消息中特殊字符（如 `*`）的过滤逻辑，确保消息显示的整洁。
