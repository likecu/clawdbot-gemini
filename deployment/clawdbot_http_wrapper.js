...
app.post('/chat', async (req, res) => {
    ...
    const callbackId = callback_session_id || sessionId;
    console.log(`[${new Date().toISOString()}] DEBUG: SessionID=${sessionId}, CallbackID=${callbackId}, Message=${message?.substring(0, 30)}...`);
                ...
                if (jsonEnd !== -1) {
                    ...
                    try {
                        ...
                        if (payloads) {
                            payloads.forEach(p => {
                                if (p.text) {
                                    sendCallback(callbackId, p.text);
                                    sentCount++;
                                    finalReply = p.text;
                                }
                            });
